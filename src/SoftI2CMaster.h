...
void i2c_read_buffer_from_register(uint8_t addr, uint8_t register_number, uint8_t * byte_buffer, uint8_t number_of_bytes_to_read);
void i2c_write_buffer_to_register(uint8_t addr, uint8_t register_number, uint8_t * byte_buffer, uint8_t number_of_bytes_to_write) ;

void i2c_read_buffer_from_16bit_register(uint8_t addr, uint16_t register_number, uint8_t * byte_buffer, uint8_t number_of_bytes_to_read);
void i2c_write_buffer_to_16bit_register(uint8_t addr, uint16_t register_number, uint8_t * byte_buffer, uint8_t number_of_bytes_to_write);

// Read word from register, low byte first
uint16_t i2c_read_word_from_register(uint8_t addr, uint8_t register_number);

// Read word from register, high byte first
uint16_t i2c_read_word_swapped_from_register(uint8_t addr, uint8_t register_number);

void i2c_write_byte_to_register(uint8_t addr, uint8_t register_number, uint8_t byte);
...
void i2c_delay_half(void) { // function call 3 cycles => 3C
...
#  endif
#else // __has_include("digitalWriteFast.h")
#  if I2C_PULLUP
  digitalWrite(SDA, 1);
  digitalWrite(SCL, 1);
#  else
  ...
  digitalWrite(SCL, 0);
#  endif
#endif // __has_include("digitalWriteFast.h")
...
#pragma GCC diagnostic pop // GCC diagnostic ignored "-Wunused-parameter"

#if !defined(_WORD_UNION_H)
/**
...
void i2c_read_buffer_from_register(uint8_t addr, uint8_t register_number, uint8_t * byte_buffer, uint8_t number_of_bytes_to_read){
    ...
    i2c_rep_start(addr | I2C_READ); // restart for reading
    int8_t i = 0;
    for (; i < number_of_bytes_to_read - 1; ++i) {
        byte_buffer[i] = i2c_read(false);
    }
    byte_buffer[i] = i2c_read(true);
}

void i2c_read_buffer_from_16bit_register(uint8_t addr, uint16_t register_number, uint8_t * byte_buffer, uint8_t number_of_bytes_to_read){
    i2c_start (addr);
    i2c_write(register_number >> 8);
    i2c_write(register_number & 0xFF);
    i2c_rep_start(addr | I2C_READ); // restart for reading
    int8_t i = 0;
    for (; i < number_of_bytes_to_read - 1; ++i) {
        byte_buffer[i] = i2c_read(false);
...
void i2c_write_buffer_to_16bit_register(uint8_t addr, uint16_t register_number, uint8_t * byte_buffer, uint8_t number_of_bytes_to_write) {
    ...
    i2c_write(register_number >> 8);
    i2c_write(register_number & 0xFF);
    for (uint8_t i = 0; i < number_of_bytes_to_write; i++) {
        i2c_write (byte_buffer[i]);
    }
    i2c_stop();
}
...
uint16_t i2c_read_word_swapped_from_register(uint8_t addr, uint8_t register_number) {
    WordUnion tWord; // Using WordUnion saves 16 bytes program space
