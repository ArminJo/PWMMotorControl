...
// only synchronize if manually operated or at full speed
        ...
        if ((MotorRampState == MOTOR_STATE_STOPPED && aOtherMotorControl->MotorRampState == MOTOR_STATE_STOPPED
            ...
            if (EncoderCountForSynchronize >= (aOtherMotorControl->EncoderCountForSynchronize + 2)) {
                EncoderCountForSynchronize = aOtherMotorControl->EncoderCountForSynchronize;
                /*
            ...
            } else if (aOtherMotorControl->EncoderCountForSynchronize >= (EncoderCountForSynchronize + 2)) {
                aOtherMotorControl->EncoderCountForSynchronize = EncoderCountForSynchronize;
                /*
...
unsigned int EncoderMotor::getDistanceCentimeter() {
    return (EncoderCount * FACTOR_COUNT_TO_MILLIMETER_INTEGER_DEFAULT) / 10;
}
...
EncoderMotor *EncoderMotor::sMotorControlListStart = nullptr;

void EncoderMotor::AddToMotorList() {
    EncoderMotor::sNumberOfMotorControls++;
    NextMotorControl = nullptr;
    if (sMotorControlListStart == nullptr) {
        // first constructor
        ...
        while (tObjectPointer->NextMotorControl != nullptr) {
...
// walk through list
    ...
    for(EncoderMotor *tEncoderMotorControlPointer = sMotorControlListStart; tEncoderMotorControlPointer != nullptr;tEncoderMotorControlPointer =
